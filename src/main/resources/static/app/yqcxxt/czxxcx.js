var czxxcxcx=(function ($) {
	$(function () {
//		$('#czxxcx-stateTime').datetimepicker(datetimeDefaultOption);
////		$('#czxxcx-stateTime').val(new Date().Format('yyyy-MM-dd hh:mm:ss'));
//		$('#czxxcx-endTime').datetimepicker(datetimeDefaultOption);
////		$('#czxxcx-endTime').val(new Date().Format('yyyy-MM-dd hh:mm:ss'));
		
		var today = new Date();
		var oneday = 1000 * 60 * 60 * 2;
		$('#czxxcx-stateTime').datetimepicker(datetimeDefaultOption);
		$('#czxxcx-endTime').datetimepicker(datetimeDefaultOption);
		// $('#czxxcx-stateTime').val(new Date(today-oneday).Format('yyyy-MM-dd hh:mm:ss'));
		// $('#czxxcx-endTime').val(new Date(today).Format('yyyy-MM-dd hh:mm:ss'));
		$('#czxxcx-stateTime').val(new Date().Format('yyyy-MM-dd 00:00:00'));
		$('#czxxcx-endTime').val(new Date(today).Format('yyyy-MM-dd hh:mm:ss'));

		$('#czxxcxgj-stateTime').datetimepicker(datetimeDefaultOption);
		$('#czxxcxgj-endTime').datetimepicker(datetimeDefaultOption);
		//地图
		var map = new AMap.Map('czxxcxMap', {
			zoom:14,
		...
		// 		    });
		// 			$("#czxxcx-cphm").empty();
		// 			$("#czxxcx-cphm").append('<option value=""></option>');
		// 			$("#czxxcx-cphm").append('<option value="null">全部车牌</option>');
		// 			for(var i=0; i<data.length;i++){
		// 				$("#czxxcx-cphm").append('<option value="'+data[i].CPHM+'">'+data[i].CPHM+'</option>');
		// 			}
		// 		},
		// 		error: function(){
		// 		}
...
// 				success:function(data){
// 					$("#czxxcx-type").empty();
// 					$("#czxxcx-type").append('<option value="" disabled selected>类型</option>');
// 					$("#czxxcx-type").append('<option value="null">全部类型</option>');
// 					// for(var i=0; i<data.length;i++){
// 						$("#czxxcx-type").append('<option value="套牌">套牌</option>');
// 					// }
// 				},
// 				error: function(){
// 				}
// 			});
			    				...
			    				itemTemplate:
			    					function(value, item) {
				    					...
				    					return [
											...
											// 	document.getElementById('czxxcxbig').src = "../../common/pic?key="+key;
											// 	imagebigscale("czxxcxbigImage", "czxxcxbig", "czxxcximagetool", "czxxcxclose", "czxxcxleft-rote", "czxxcxright-rote", "czxxcxbigger", "czxxcxsmaller");
											// 	document.getElementById('czxxcxbigImage').style.display = 'block';
											// 	document.getElementById('czxxcxbigImage').style.top = '0px';
											// 	document.getElementById('czxxcxbigImage').style.left = '30px';
											// }),
											$('<a>').addClass('btn btn-primary btn-sm').text('轨迹').on('click', function () {
												...
												$('#czxxcxgj-stateTime').val(stime);
												$('#czxxcxgj-endTime').val(etime);
		...
		$('#czxxcx-select').on('click', function () {
			findczxxcx();
		});

		var vehicle_no="";
		$('#czxxcxgj-select').on('click', function () {
			findGj(vehicle_no);
		});
		...
		function findGj(vehicle){
			...
			if($('#czxxcxgj-stateTime').val() == ""||$('#czxxcxgj-endTime').val()==""){
			...
			jqxhr=$.ajax({
				...
				url:"../../claq/findGj",
				data:{
					"stime" : $('#czxxcxgj-stateTime').val(),
					"etime" : $('#czxxcxgj-endTime').val(),
					"vehino" : vehicle
				},
				dataType: 'json',
				timeout : 3600000,
...
//			setTimeout("findczxxcx()",1000);
		...
		function re(filter, callback){
            ...
            jqxhr=jqxhr=$.ajax({
            	type: "POST",
				url: "../../yqcx/findczxxcx",
				data:{"stime":$("#czxxcx-stateTime").val(),
					"etime":$("#czxxcx-endTime").val(),
					"vehicle":$("#czxxcx-cphm").val(),
					"company":$("#czxxcx-gs").val(),
					"phone": $("#czxxcx-phone").val(),
...
//        			alert("数据异常");
		...
		findczxxcx();
		$('#czxxcx-reset').on('click', function () {
			$('.panel-queryBar .form-control').val('');
			$('.panel-queryBar .select2').val([""]).trigger("change");
		});
		$('#czxxcx-dc').on('click', function () {
			layer.confirm('你确定要导出数据', {
				btn: ['确定', '取消'],
				offset: '100px'
			}, function (layerIndex) {
				window.open(basePath+"yqcx/findczxxcxdc?stime="+$("#czxxcx-stateTime").val()
						+"&etime="+$("#czxxcx-endTime").val()
						+"&vehicle="+$("#czxxcx-cphm").val()
						+"&company="+$("#czxxcx-gs").val()
						+"&phone="+$("#czxxcx-phone").val()
