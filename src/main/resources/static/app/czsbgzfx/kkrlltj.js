var kkrlltj=(function(Vue, _, $) {
	...
	$(function () {
		...
		$('#kkrlltj-stateTime').datetimepicker(dateDefaultOption);
		$('#kkrlltj-stateTime').val(new Date(new Date()-7*24*3600*1000).Format('yyyy-MM-dd'));
//		$('#kkrlltj-stateTime').val(getMyDate(new Date()));
		$('#kkrlltj-endTime').datetimepicker(dateDefaultOption);
		$('#kkrlltj-endTime').val(new Date().Format('yyyy-MM-dd'));
		...
		jqxhr=$.ajax({
			...
			dataType: 'json',
			success:function(data){
				$("#kkrlltj-address").empty();
				$("#kkrlltj-address").append('<option value="" disabled selected>场站</option>');
				$("#kkrlltj-address").append('<option value="null">全部</option>');
				for(var i=0; i<data.length;i++){
					if(null!=data[i].ADDRESS){
						$("#kkrlltj-address").append('<option value="'+data[i].ADDRESS+'">'+data[i].ADDRESS+'</option>');						
					}
				}
			},
			error: function(){
		...
		$('#kkrlltj-select').on('click', function () {
			findkkrlltj();
		});
...
//			}
//		}	
		function findkkrlltj(){
//			var str1=$('#kkrlltj-stateTime').val();
//  			var str2=$('#kkrlltj-endTime').val();
//  			if(issh(str1,str2)=='1'){
...
//			}
            ...
            jqxhr=$.ajax({
            	type: "POST",
				url: "../../zpsj/findkkrlltj",
				data:{"stime":$("#kkrlltj-stateTime").val(),
					"etime":$("#kkrlltj-endTime").val(),
					"address":address,
					"check":$("#check").val(),
		...
		function drawEcharts(c,d){			
			echart = echarts.init(document.getElementById("kkrlltjEchart"));
			echart.setOption ({
...
//			}
			...
			layer.confirm('你确定要导出数据', {
				...
				window.open(basePath+"zpsj/findkkrlltjdc?stime="+$("#kkrlltj-stateTime").val()
						+"&etime="+$("#kkrlltj-endTime").val()
						+"&address="+address
						+"&check="+$("#check").val()
				);
