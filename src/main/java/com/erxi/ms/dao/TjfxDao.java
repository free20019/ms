...
//				tj += " and b.VEHICLE_NO like '%"+xm+"%'";
			...
			String sql = "select (select count(*) count  from tb_taxi_gzfx_history  b "
					+ " left join (select * from tb_global_vehicle where industry=090 and business_scope_code = 1400  AND STATUS_NAME='营运' AND PLATE_NUMBER LIKE '浙A%') v on v.PLATE_NUMBER = b.VEHICLE_NO where 1=1 ";
			sql += tj;
			sql+=" ) as count, tt.* from (select b.*,v.company_name COMP_NAME from tb_taxi_gzfx_history b "
					+ " left join (select * from tb_global_vehicle where industry=090 and business_scope_code = 1400  AND STATUS_NAME='营运' AND PLATE_NUMBER LIKE '浙A%') v on v.PLATE_NUMBER = b.VEHICLE_NO where 1 = 1 ";
			sql += tj;
			sql += "  ) tt limit "+((pageIndex-1)*pageSize)+","+pageSize;
...
//				tj += " and b.VEHICLE_NO like '%"+xm+"%'";
			...
			String sql = "select b.*,v.company_name COMP_NAME from tb_taxi_gzfx_history b "
					+ " left join (select * from tb_global_vehicle where industry=090 and business_scope_code = 1400  AND STATUS_NAME='营运' AND PLATE_NUMBER LIKE '浙A%') v on v.PLATE_NUMBER = b.VEHICLE_NO where 1 = 1 ";
			sql += tj;
			System.out.println(sql);
