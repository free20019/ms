...
public interface TlaqDao {
	 ...
	 * @return
	 */
	@Insert("insert into TB_ZDGZCL_TAXI (vehicle, type) values (CASE WHEN EXISTS(SELECT t.* FROM (SELECT * FROM TB_ZDGZCL_TAXI WHERE vehicle='${vehi}') t ) THEN '1' ELSE '${vehi}' END, ${type})")
	public int insExcelVehi(@Param("vehi") String vehi,@Param("type") String type);
	 ...
	 * 查询车辆
	 */
	@Select("select t.BRAND,t.MODEL,count(t.brand)as count from TB_GLOBAL_VEHICLE t where t.industry=090 and t.business_scope_code = 1400  AND t.STATUS_NAME='营运' and t.plate_number LIKE '浙A%' group by t.brand,t.MODEL order by count desc")
	@ResultType(String.class)
	...
	@Select("select case when (to_days(now()) - to_days(str_to_date(date_format(PURCHASE_DATE, '%Y-%m-%d'), '%Y-%m-%d'))) < 365 then '0(含)至1年' "+
       ...
       "end as ti,count(*) as count from TB_GLOBAL_VEHICLE where industry=090 and business_scope_code = 1400  AND STATUS_NAME='营运' and plate_number LIKE '浙A%' group by case "+
...
//	@Select("select t.PLATE_NUMBER,t.ID from TB_GLOBAL_vehicle t where industry=090 and business_scope_code = 1400  AND STATUS_NAME='营运' AND PLATE_NUMBER LIKE '浙A%'")
	@ResultType(String.class)
