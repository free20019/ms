...
//				_this.getxll();
	...
	$(function () {
		$('#kkzpsjcx-stateTime').datetimepicker(datetimeDefaultOption);
		$('#kkzpsjcx-endTime').datetimepicker(datetimeDefaultOption);
		$('#kkzpsjcx-stateTime').val(new Date().Format('yyyy-MM-dd 00:00:00'));
		$('#kkzpsjcx-endTime').val(new Date().Format('yyyy-MM-dd 23:59:59'));
		$(".select2").select2({
		...
		$('#kkzpsjcx-option').on('change', function () {
			if ($('#kkzpsjcx-option').val() == '1') {
				$('#kkzpsjcx-stateTime').datetimepicker('remove');
				$('#kkzpsjcx-endTime').datetimepicker('remove');
				$('#kkzpsjcx-stateTime').datetimepicker(datetimeDefaultOption);
				$('#kkzpsjcx-endTime').datetimepicker(datetimeDefaultOption);
				$('#kkzpsjcx-stateTime').val(new Date().Format('yyyy-MM-dd 00:00:00'));
				$('#kkzpsjcx-endTime').val(new Date().Format('yyyy-MM-dd 23:59:59'));
				...
				$('#kkzpsjcx-stateTime').datetimepicker('remove');
				$('#kkzpsjcx-endTime').datetimepicker('remove');
				$('#kkzpsjcx-stateTime').datetimepicker(dateDefaultOption);
				$('#kkzpsjcx-endTime').datetimepicker(dateDefaultOption);
				if ($('#kkzpsjcx-option').val() == 'null') {
					$('#kkzpsjcx-stateTime').val(new Date().Format('yyyy-MM-dd'));
					$('#kkzpsjcx-endTime').val(new Date().Format('yyyy-MM-dd'));
				} else {
					$('#kkzpsjcx-stateTime').val(new Date(new Date() - 1000 * 60 * 60 * 24).Format('yyyy-MM-dd'));
					$('#kkzpsjcx-endTime').val(new Date(new Date() - 1000 * 60 * 60 * 24).Format('yyyy-MM-dd'));
				}
			}
		});
//		jqxhr = $.ajax({
//			type: "POST",
//			url: "../../zpsj/xll",
//			data: {"field": "VEHICLE_NO", "table": "tb_vehicle_hk2"},
//			timeout: 180000,
//			dataType: 'json',
//			success: function (data) {
...
//		});
					...
					function (value, item) {
						...
						return [
							$('<a>').addClass('btn btn-primary btn-sm').text('详情').on('click', function () {
								...
								document.getElementById('zpsjbig').src = "../../common/pic2?key=" + key;
		...
		$('#kkzpsjcx-select').on('click', function () {
			findkkzpsjcx();
		});
		function findkkzpsjcx() {
			var all = 0;
			$('#kkzpsjcxTable').jsGrid({
				width: '100%',
		...
		function re(filter, callback) {
			...
			// 	check = '1';
			// }
//			var options = document.getElementById("kkzpsjcx-address");
//			var address = "";
...
//			address = address.substr(0, (address.length - 1));
			...
			jqxhr = $.ajax({
				type: "POST",
				url: "../../zpsj/findkkzpsjcx",
				data: {
					"stime": $("#kkzpsjcx-stateTime").val(),
					"etime": $("#kkzpsjcx-endTime").val(),
					"vehicle": $("#kkzpsjcx-cphm").val(),
					"address": address,
					"company": $("#kkzpsjcx-gs").val(),
					"type": $('#kkzpsjcx-option').val(),
					"check": $('#check').val(),
		...
		$('#kkzpsjcx-dc').on('click', function () {
			...
			layer.confirm('你确定要导出数据', {
				...
				window.open(basePath + "zpsj/findkkzpsjcxdc?stime=" + $("#kkzpsjcx-stateTime").val()
					+ "&etime=" + $("#kkzpsjcx-endTime").val()
					+ "&vehicle=" + $("#kkzpsjcx-cphm").val()
					+ "&address=" + address
					+ "&company=" + $("#kkzpsjcx-gs").val()
