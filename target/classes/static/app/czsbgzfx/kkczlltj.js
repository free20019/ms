var kkczlltj=(function (Vue, _, $) {
	...
	$(function () {
		...
		$('#kkczlltj-stateTime').datetimepicker(datetimeDefaultOption);
		$('#kkczlltj-stateTime').val(new Date().Format('yyyy-MM-dd 00:00:00'));
		$('#kkczlltj-endTime').datetimepicker(datetimeDefaultOption);
		$('#kkczlltj-endTime').val(new Date().Format('yyyy-MM-dd 23:59:59'));
		...
		$('#kkczlltj-select').on('click', function () {
			findkkczlltj();
		});
...
//			}
//		}	
		function findkkczlltj(){
//			var str1=$('#kkczlltj-stateTime').val();
//  			var str2=$('#kkczlltj-endTime').val();
//  			if(issh(str1,str2)=='1'){
//  				layer.msg('不支持跨越查询',{icon:2});
//  				return false;
//  			}
			var all = 0;
			$('#kkczlltjTable').jsGrid({
				width: '100%',
...
//			}
            ...
            jqxhr=$.ajax({
            	type: "POST",
				url: "../../zpsj/findkkczlltj",
				data:{"stime":$("#kkczlltj-stateTime").val(),
					"etime":$("#kkczlltj-endTime").val(),
					"address":address,
					"check":$("#check").val(),
		...
		function drawEcharts(c,d){			
			echart = echarts.init(document.getElementById("kkczlltjEchart"));
			echart.setOption ({
...
//			}
			...
			layer.confirm('你确定要导出数据', {
				...
				window.open(basePath+"zpsj/findkkczlltjdc?stime="+$("#kkczlltj-stateTime").val()
						+"&etime="+$("#kkczlltj-endTime").val()
						+"&address="+address
						+"&check="+$("#check").val()
				);
