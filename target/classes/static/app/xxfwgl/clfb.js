	...
	$(function () {
		var vm = new Vue({
			...
			methods: {
				QyjkDsq: function () {
					var _this = this;
					if(this.clfbdsq) clearInterval(this.clfbdsq)
					this.clfbdsq = setInterval(function(){
				...
				getQyjk: function () {
					...
					$.ajax({
						...
						_this.areaList = Object.freeze(res.arealist);
						_this.vehicleList = Object.freeze(res.vehilist);
						init();
					});
				},
				...
				handleAreaVehicleReturnSingleClick:function(item){
					...
					for(var i=0;i<a.length;i++){
						if(item.vehicle == a[i].vehino){
							...
							s.carState = b.carState;
							
							if (b.lati == null || "" == b.lati || ""==b.longi) {
								s.statue = "离线";
							} else {
								if (b.onoffstate=="1") {
	 								if (b.carStatus=="0") {
...
// 				}else{
		function init(){
 					...
 					var ons=0,offs=0,busys=0,emptys=0,off1s=0,off2s=0,off3s=0,todayOnlines=0;
 					var on = new Array(), 
 					off = new Array(),
 					busy = new Array(),
 					empty = new Array(),
...
// 						}
 						...
 						if(ve.dateTime.substring(0,10)==new Date().Format('yyyy-MM-dd').toString()){
							todayOnlines++;
						}
 						all.push(s);
 					}
 					...
 					$('.todayOnlineNumber').html(todayOnlines);
 					$('.todayOnlineRate').html((todayOnlines/clgzvehilist.length*100).toFixed(2)+"%");
...
//			            	return null;
		    	...
		    	pointSimplifierIns.setData(obj);
		    	clfb_map.setZoomAndCenter(14,[120.209562,30.245278])
